...
plotpath = "data_new/plots/"


create_plots = False # True/False --> Sollen plots gemacht werden? (Programmlaufzeit +)
...
def diff_geraden_XY(dataset,a,b):
    """
    ...
    """
    soll = np.array(dataset["x"])
    ist = a*np.array(dataset["x"])+b
    diff = np.abs(soll-ist)
    diff_mean = np.mean(diff)
    return(diff_mean)
...
def plot_beschleunigung(acc_time, acc_x, acc_y, acc_z, fname):
    if create_plots == True:
        ...
        plt.savefig(f"{plotpath}Beschleunigung/{fname}_Beschleunigung.png")
        plt.show()
...
def plot_orientierung(o_time, o_alpha, o_beta, o_gamma, fname):
    if create_plots == True:
        ...
        plt.savefig(f"{plotpath}Orientierung/{fname}_Orientierung.png")
        plt.show()
...
def plot_geschwindigkeit(delta_t_acc, acc_v_x, acc_v_y, acc_v_z, fname):
    if create_plots == True:
        ...
        plt.savefig(f"{plotpath}Geschwindigkeit/{fname}_Geschwindigkeit.png")
        plt.show()
...
def plot_trajektorie_xy(dataset, mx, my, fname="XY-Trajektorie.png"):
    if create_plots == True:
        ...
        plt.savefig(f"{plotpath}XY_Trajektorie/{fname}_XY-Trajektorie.png")
        plt.show()
...
def plot_trajektorie_xyz(trajectorie_xyz, fname):
    if create_plots == True:
        ...
        ax.view_init(20, 45)
        plt.savefig(f"{plotpath}XYZ_Trajektorie/{fname}_XYZ-Trajektorie.png")
        plt.show()
...
def plot_kreis_XY(dataset, kreis, mx, my, fname):
    if create_plots == True:
        ...
        plt.savefig(f"{plotpath}Kreise/{fname}_Kreis.png")
        plt.show()
        
def plot_geraden_XY(dataset,a,b,fname):
    if create_plots == True:
        fig = plt.figure()
        ax = plt.axes()
        ax.plot(dataset["y"],dataset["x"],'o',label='Messpunkte')
        ax.plot(dataset["y"],a*np.array(dataset["y"])+b, label='Ausgleichende Gerade')
        plt.title('Ausgleichende Gerade')
        plt.legend()
        plt.savefig(f"{plotpath}Geraden/{fname}_Geraden.png")
        plt.show()
#%% 
"""
##### Hauptprogramm #####
...
def processing(filename, log_Abweichungen = True):
    ...
    # Berechnung und Visualisierung einer Ausgleichenden Gerade in die XY-Trajektorie
    a, b, r, p, std = linregress(dataset["y"],dataset["x"])
    plot_geraden_XY(dataset, a, b, filename)
    
    standardabweichung_geraden = std
    #standardabweichung_geraden = diff_geraden_XY(dataset,a,b)

    # Berechnung der Standardabweichung zum Kreis
    mittlere_abweichung_kreis = diff_kreis_XY(trajectorie_xy, kreis, mx, my)
    if log_Abweichungen == True:
        print(f"{filename}, Abweichung: {np.round(mittlere_abweichung_kreis,3)}[m]")
    return(mittlere_abweichung_kreis, standardabweichung_geraden, np.sum(delta_t_acc))

#%% 
"""
##### Abruf des Hauptprogramms für alle Dateien #####
"""
d_kreis_kreise = np.mean([processing(filename, log_Abweichungen = False)[0] for filename in filelist_kreise])
d_kreis_kreuze  = np.mean([processing(filename, log_Abweichungen = False)[0] for filename in filelist_kreuze])
create_plots = False

d_geraden_kreise = np.mean([processing(filename, log_Abweichungen = False)[1] for filename in filelist_kreise])
dauer_kreise = np.mean([processing(filename, log_Abweichungen = False)[2] for filename in filelist_kreise])

d_geraden_kreuze = np.mean([processing(filename, log_Abweichungen = False)[1] for filename in filelist_kreuze])
dauer_kreuze = np.mean([processing(filename, log_Abweichungen = False)[2] for filename in filelist_kreuze])

print(f"\nMittlere Abweichungen vom Kreis:\nKreise: {np.mean(d_kreis_kreise)}[m]\nKreuze: {np.mean(d_kreis_kreuze)}[m]")
print(f"\nMittlere Abweichungen von Linie:\nKreise: {np.mean(d_geraden_kreise)}[m]\nKreuze: {np.mean(d_geraden_kreuze)}[m]")
print(f"\nMittlere Zeichendauer:\nKreise: {np.mean(dauer_kreise)}[s]\nKreuze: {np.mean(dauer_kreuze)}[s]")

#%% 
"""
##### Abruf des Hauptprogramms für eine einzelne Datei #####
"""
print(processing("data/Andere/sensorData (0).txt"))